...
from conf import *
from node_selection_policies import CustomNodeSelector


def perform_SCIP_instance(instance_path, node_comp="estimate", node_select="BFS", parameter_settings=False,
                          time_limit=None):
    model = Model()
    model.hideOutput()
    ...
    if node_comp != "SCIP":
        oracle_ns = CustomNodeSelector(comp_policy=node_comp, sel_policy=node_select)
        model.includeNodesel(oracle_ns, "", "",
                             536870911, 536870911)
    model.optimize()
    if time_limit is not None:
...
def perform_SCIP_instances_using_a_tuned_comp_policy(instances_folder="", node_comp="estimate", node_select="BFS",
                                                     nb_max=None,
                                                     printing=False,
    ...
    for instance in os.listdir(instances_folder):
        if instances_indexes is None or index_of_the_folder in instances_indexes:
            if instance.endswith(".lp") or instance.endswith(".mps"):
                ...
                visited_nodes, time_or_gap = perform_SCIP_instance(instance_path, node_comp, node_select,
