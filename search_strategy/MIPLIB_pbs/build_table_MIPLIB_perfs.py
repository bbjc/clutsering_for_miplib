            ...
            for time_limit in conf.time_limits:
                ...
                if extracted_infos is not None:
                    ...
                    if partition == "test":
                        ...
                        folder = os.path.join(conf.ROOT_DIR, f"data\\MIPLIB2017\\miplib")
...
def stats_comparison_GP_SCIP(table_size="full"):
        ...
        for function in functions:
            ...
            for time_limit in conf.time_limits:
                ...
                if extracted_infos is not None:
                    ...
                    list_of_feasibles=[]
                    if partition == "test":
                        csv_path = f"search-strategy-generation-for-branch-and-bound-using-genetic-programming/cluster_csv/hc_complete_method.csv"
                        df = pd.read_csv(csv_path)
                        cluster_1 = df[df["cluster"] == 1]["Instance"].tolist()
                        ...
                        folder = f"search-strategy-generation-for-branch-and-bound-using-genetic-programming/data/MIPLIB2017/miplib/"
                        instances_set = os.listdir(folder)
                    for instance in instances_set:
    ...
    if table_size == "full":
        ...
        for function in functions:
            ...
            for partition in ["test","transfer"]:
                ...
                for time_limit in considered_times_for_table:
                    ...
                    c_gap, m_gap, y_gap, k_gap = conf.gradient_color(
                        transformed_info[partition][function][time_limit]["mean_geo"], mini_gap[partition][time_limit],
                        mini_gap[partition][time_limit] * 1.4)
                    text += f' & \\textcolor[cmyk]{{{c_inf},{m_inf},{y_inf},{k_inf}}}{{$ {transformed_info[partition][function][time_limit]["unfeasiblies"]} $}} & \\textcolor[cmyk]{{{c_gap},{m_gap},{y_gap},{k_gap}}}{{$ {transformed_info[partition][function][time_limit]["mean_geo"]} \\pm {transformed_info[partition][function][time_limit]["std_geo"]} $}}'
            ...
            text += '\\\\'
            with open("latexfile.txt", "w") as file:
                file.write(text)
            print("###########TEXT############")
            print(text)
