...
def building_all_batches():
    saving_folder = os.path.join(conf.ROOT_DIR,
                                 'evaluation_of_all_methods_cluster/seed/')
    for partition, nb_of_instances in zip(["test", "transfer"], [166, 758]):
        for time_limit in conf.time_limits:
            ...
            for seed in conf.gp_funcs_MIPLIB_seeds.keys():
                to_print = sh_template + f'python -u search-strategy-generation-for-branch-and-bound-using-genetic-programming/MIPLIB_pbs/evaluation_miplib_pb.py -function seed_{seed} -time_limit {time_limit} -partition {partition}'
            ...
            to_print = sh_template + f'python -u search-strategy-generation-for-branch-and-bound-using-genetic-programming/MIPLIB_pbs/evaluation_miplib_pb.py -function SCIP -time_limit {time_limit} -partition {partition}'
...
def build_batch_jobs_handcrafted_heuristics():
    saving_folder = os.path.join(conf.ROOT_DIR,
                                 'evaluation_of_all_methods_cluster/')
        ...
        #for time_limit in conf.time_limits:
            if time_limit not in conf.time_limits_for_paper:
                ...
                for function in ["best_estimate_BFS","best_estimate_DFS"]:#,"best_LB_BFS"
                    to_print = sh_template + f'python -u search-strategy-generation-for-branch-and-bound-using-genetic-programming/MIPLIB_pbs/evaluation_miplib_pb.py -function {function} -time_limit {time_limit} -partition {partition}'
