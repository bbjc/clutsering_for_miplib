...
def evaluate_a_function_and_store_it(problem, function, performance_folder,
    ...
    if func_name is None:
        new_json_dir = os.path.join(os.path.abspath('')[:len(os.path.abspath(''))],  # - 25
                                    f'simulation_outcomes/{problem}/{partition}_{function}.json')
        print("perfs are done for the function ", function)
    else:
        new_json_dir = os.path.join(os.path.abspath('')[:len(os.path.abspath(''))],  # - 25
                                    f'simulation_outcomes/{problem}/{partition}_{func_name}.json')
...
def evaluate_the_GP_heuristics_and_SCIP_functions(problem, partition="test", GP_dics=None,
                                                  parameter_settings=False,instances=[]):
    folder = f"data/{problem}/{partition}/"
    if GP_dics is not None:
        for key in GP_dics.keys():
            evaluate_a_function_and_store_it(problem, GP_dics[key], folder, partition,
    ...
    for function in functions:
        evaluate_a_function_and_store_it(problem, function, folder, partition,
...
if __name__ == "__main__":
    ...
    evaluate_the_GP_heuristics_and_SCIP_functions(problem, partition=partition, GP_dics=gp_func_dic, parameter_settings=True,instances=[instance])
